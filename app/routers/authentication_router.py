...
async def register(form_data: RegisterUserScheme):
    try:
        user = await User.filter(username=form_data.username, email=form_data.email).first()

        if user is not None:
            return JSONResponse(status_code=400, content={"message": "Already registered user"})
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    is_user_right = Password.verify_password(form_data.password, user.hashed_password)
